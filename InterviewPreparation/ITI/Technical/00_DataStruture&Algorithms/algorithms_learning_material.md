...
> **How to use this guide:** Read sections in order if you're new to algorithms. If you already know the basics, jump to any section using the Table of Contents. Each algorithm section is self-contained with code you can run immediately.
...
9. [Advanced Techniques](#9-advanced-techniques)
   - Two Pointers, Sliding Window, Binary Search on Answer, Backtracking
...
> 1. How many steps does binary search take on an array of 1,000,000 elements? (Hint: log₂(10⁶) ≈ ?)
> 2. Can you modify binary search to find the number of times a target appears in a sorted array?
> 3. Why must the array be sorted for binary search to work? Give an example where it fails on an unsorted array.
...
## 6.4 Minimum Spanning Tree: Kruskal's Algorithm
...
## 6.5 Minimum Spanning Tree: Prim's Algorithm
...
> 2. Why can't Dijkstra's algorithm handle negative edge weights?
> 3. A graph has 5 vertices and 7 edges. How many edges will its MST have?
> 4. Modify BFS to detect if a graph is bipartite (can be 2-colored).
...
## 9.3 Binary Search on the Answer
...
> 1. Use Two Pointers to check if a string is a palindrome, ignoring non-alphanumeric characters.
> 2. Modify the sliding window for "Longest Substring with at most K distinct characters."
> 3. Use binary search on the answer to solve: "Given a sorted array, find the square root of x without using math.sqrt, rounded down."
> 4. Use backtracking to generate all valid combinations of parentheses for n pairs (e.g., n=3 → "((()))", "(()())", etc.).
